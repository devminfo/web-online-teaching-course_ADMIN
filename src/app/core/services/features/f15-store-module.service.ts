...
get(): Observable<any> {
  return this.http
    .get<any>(this.apiURL + '/store-modules')
...
delete(id: any) {
  return this.http
    .delete<any>(this.apiURL + '/store-modules/' + id, this.httpOptions)
    .pipe(retry(1), catchError(this.handleError));
}
...
deleteManyByIds(ids: any) {
  return this.http
    .delete<any>(
      `${this.apiURL}/store-modules/${ids}/ids`,
      this.httpOptions
    )
    .pipe(retry(1), catchError(this.handleError));
}
...
find(id: any): Observable<any> {
  return this.http
    .get<any>(this.apiURL + '/store-modules/' + id)
    .pipe(retry(1), catchError(this.handleError));
}
...
paginate(params: {
  ...
  let url = `${this.apiURL}/store-modules/paginate?page=${params.page}&limit=${params.limit}&${params.filter}`;
