...
export class SpecializeService {
  ...
  get(): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/specializes')
  ...
  delete(id: any) {
    return this.http
      .delete<any>(this.apiURL + '/specializes/' + id, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  find(id: any): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/specializes/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  paginate(page: number, limit: number, filter: any): Observable<any> {
    let url =
      this.apiURL + '/specializes/paginate?page=' + page + '&limit=' + limit;

    // add condition filter
    if (filter != '') {
      url =
        this.apiURL +
        '/specializes/paginate?page=' +
        page +
        '&limit=' +
        limit +
        filter;
