...
export class GroupAPIService {
  ...
  get(): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/group-apis')
  ...
  delete(id: any) {
    return this.http
      .delete<any>(this.apiURL + '/group-apis/' + id, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  find(id: any): Observable<any> {
    return this.http
      .get<any>(this.apiURL + '/group-apis/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  paginate(page: number, limit: number, filter: any): Observable<any> {
    let url =
      this.apiURL + '/group-apis/paginate?page=' + page + '&limit=' + limit;

    // add condition filter
    if (filter != '') {
      url =
        this.apiURL +
        '/group-apis/paginate?page=' +
        page +
        '&limit=' +
        limit +
        filter;
