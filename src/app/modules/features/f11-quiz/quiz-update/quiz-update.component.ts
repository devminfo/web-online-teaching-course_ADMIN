...
import { QuizService } from 'src/app/core/services/features/f11-quiz.service';
@Component({
  selector: 'app-quiz-update',
  templateUrl: './quiz-update.component.html',
  styleUrls: ['./quiz-update.component.scss'],
})
export class QuizUpdateComponent implements OnInit, AfterViewInit, OnDestroy {
  // subscription
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(
        this.api.update(this.id, this.input).subscribe(() => {
          ...
          // redirect to list
          this.router.navigate(['/features/quizs']);
        })
      );
    }
  }
